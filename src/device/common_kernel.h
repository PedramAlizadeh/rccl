...
__device__ __forceinline__ void reduceCopyPacks(
  ...
  BytePack<BytePerPack> acc1[MinSrcs + !MinSrcs];
  BytePack<BytePerPack> acc2[MinSrcs + !MinSrcs];
  bool tail_processing = false;

  if (Unroll==1 ? (BytePerPack <= threadBytesAhead) : (0 < nHunksAhead)) {
    for (int s=0; s < MinSrcs; s++) {
      ...
      // for (int u=0; u < Unroll; u++) {
        ...
        // } else {
          ...
          acc1[s] = ld_volatile_global<BytePerPack>(minSrcs[s]);
          minSrcs[s] += WARP_SIZE*BytePerPack;
          // if (0 < PreOpSrcs) acc[u] = applyPreOp(preFn, acc[u]);
        // }
    }
    tail_processing = true;
    // minSrcs[s] += WARP_SIZE*BytePerPack;
    threadBytesBehind += BytePerHunk;
    threadBytesAhead -= BytePerHunk;
    nHunksAhead -= 1;
      // }
  }
  ...
  while (Unroll==1 ? (BytePerPack <= threadBytesAhead) : (0 < nHunksAhead)) {
    ...
    for (int s=0; s < MinSrcs; s++) {
      ...
      // for (int u=0; u < Unroll; u++) {
        ...
        // } else {
          // Use volatile loads in case credits are polled for with volatile (instead of acquire).
          acc2[s] = ld_volatile_global<BytePerPack>(minSrcs[s]);
          // minSrcs[s] += WARP_SIZE*BytePerPack;
          minSrcs[s] += (2*(nWarps-1)+1)*BytePerHunk;
          // if (0 < PreOpSrcs) acc[u] = applyPreOp(preFn, acc[u]);
        // }
    }
    // minSrcs[0] cannot be nullptr so we always process it
    RedFn preFn(0 < PreOpSrcs ? preOpArgs[0] : 0);
    if (0 < PreOpSrcs) acc1[0] = applyPreOp(preFn, acc1[0]);
    ...
    for (int s=1; s < MinSrcs; s++) {
      ...
      // for (int u=0; u < Unroll; u++) {
        // coverity[dead_error_line]
        if (s < PreOpSrcs) acc1[s] = applyPreOp(preFn, acc1[s]);
        acc1[0] = applyReduce(redFn, acc1[0], acc1[s]);
      // }
    }
    ...
    if (postOp) {
      // #pragma unroll Unroll
      // for (int u=0; u < Unroll; u++)
        acc1[0] = applyPostOp(redFn, acc1[0]);
    }
    ...
    for (int d=0; d < MinDsts; d++) {
      ...
      // for (int u=0; u < Unroll; u++) {
        ...
        // } else {
          st_global<BytePerPack>(minDsts[d], acc1[0]);
        // }
        minDsts[d] += WARP_SIZE*BytePerPack;
      // }
    }
    threadBytesBehind += (2*(nWarps-1)+1)*BytePerHunk;
    threadBytesAhead -= (2*(nWarps-1)+1)*BytePerHunk;
    nHunksAhead -= 2*(nWarps-1)+1;
    if (Unroll==1 ? (BytePerPack <= threadBytesAhead) : (0 < nHunksAhead)) {
      for (int s=0; s < MinSrcs; s++) {
            acc1[s] = ld_volatile_global<BytePerPack>(minSrcs[s]);
            minSrcs[s] += WARP_SIZE*BytePerPack;
      ...
      tail_processing = false;
    }
    // RedFn preFn(0 < PreOpSrcs ? preOpArgs[0] : 0);
    if (0 < PreOpSrcs) acc2[0] = applyPreOp(preFn, acc2[0]);
    for (int s=1; s < MinSrcs; s++) {
      RedFn preFn(s < PreOpSrcs ? preOpArgs[s] : 0);
      if (s < PreOpSrcs) acc2[s] = applyPreOp(preFn, acc2[s]);
      acc2[0] = applyReduce(redFn, acc2[0], acc2[s]);
    }
    if (postOp) {
      acc2[0] = applyPostOp(redFn, acc2[0]);
    }
    for (int d=0; d < MinDsts; d++) {
      st_global<BytePerPack>(minDsts[d], acc2[0]);
      minDsts[d] += (2*(nWarps-1)+1)*BytePerHunk;
    }
